    ...
    *,
    ...
    while True:
        ...
        for index, raw_item in enumerate(items):
            ...
            # Record the normalized quote for every row, no-trade ones included, so the
            # market diagnostics can tell "no trade" apart from "not retrieved".
            observations.append(
                MarketObservation(
                    code=code[-2:].lower() + "." + code[:6],
                    reported_change_pct=_optional_decimal(
                        item.get("price_change_ratio_pct"),
                        f"prices snapshot {code}.price_change_ratio_pct",
                    ),
                    last_price=_optional_decimal(
                        item.get("last_price"), f"prices snapshot {code}.last_price"
                    ),
                    reference_price=_optional_decimal(
                        item.get("prev_price"), f"prices snapshot {code}.prev_price"
                    ),
                    volume=volume,
                    amount=turnover,
                    response_id=source.request_id,
                )
            )
