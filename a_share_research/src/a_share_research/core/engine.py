...
def _evaluate_candidate(
    ...
    target_session = parse_datetime(snapshot.data["as_of"],"snapshot.as_of").astimezone(
        ZoneInfo("Asia/Shanghai")).date().isoformat()
    own_session = any(
        e.get("provider",{}).get("frequency") == "1d"
        and e.get("instrument",{}).get("code") == security_code(candidate["security_id"])
        and e.get("latest",{}).get("date") == target_session
        and parse_datetime(e["as_of"],"evidence.as_of").astimezone(ZoneInfo("Asia/Shanghai")).date().isoformat() == target_session
        and parse_datetime(e["as_of"],"evidence.as_of").astimezone(ZoneInfo("Asia/Shanghai")).hour >= 15
        and parse_datetime(e["as_of"],"evidence.as_of") <= request.decision_at
        for e in structured_market
    )
    ...
    gates = {
        ...
        "market_freshness": (readiness == "CURRENT_SESSION" and own_session) or snapshot.data_mode == "fixture",
    }
