...
export type ResearchHistoryArgs = {
  evaluation_at: string
  limit?: number
...
export type ResearchHistoryResult = JsonObject | CliErrorResult
...
function requireResultString(value: unknown, field: string): string {
  const result = getString(value, field)
  if (result === undefined) throw new Error(`${field} is required in the CLI result`)
  return result
}

function requireResultInteger(value: unknown, field: string): number {
...
function normalizeResearchHistoryArgs(args: unknown): {
  evaluation_at: string
  limit: number
  candidate_ids?: string[]
} {
  ...
  rejectUnknownFields(
    raw,
    ['evaluation_at', 'limit', 'candidate_ids'],
  ...
  const candidateIds = (raw.candidate_ids as unknown[] | undefined)?.map((value, index) => {
  ...
  if (
    ...
    && (candidateIds.length > 50 || new Set(candidateIds).size !== candidateIds.length)
  ) {
  ...
  return { evaluation_at: evaluationAt, limit, candidate_ids: candidateIds }
}
...
export async function callResearchCli(
  command: 'run' | 'artifact-read' | 'outcome-history' | 'research-history',
  request: JsonObject,
...
export function sanitizeResearchHistoryResult(raw: unknown): ResearchHistoryResult {
  ...
  const candidates = payload.candidates.slice(0, 50).map((rawCandidate, index) => {
    ...
    if (!['active', 'deprioritize', 'close_review', 'closed'].includes(agingAction)) {
    ...
    return {
      ...
      state_or_gap_transition_count: requireResultInteger(
        candidate.state_or_gap_transition_count,
        'state_or_gap_transition_count',
      ),
      consecutive_continue_research: requireResultInteger(
        candidate.consecutive_continue_research,
        'consecutive_continue_research',
      ),
  ...
  return omitUndefinedProperties({
    ...
    candidate_filter_count: payload.candidate_filter_count === null
      ? null
      : requireResultInteger(payload.candidate_filter_count, 'candidate_filter_count'),
...
function renderResearchRun(value: ResearchRunResult): string {
  ...
  if (value.focus?.length) {
    ...
    for (const item of value.focus.slice(0, 5)) {
      ...
      lines.push(
        `  attention=${String(item.attention_score ?? 'UNKNOWN')} `
        + `bucket=${item.attention_bucket ?? 'UNKNOWN'} view=${item.opportunity_view ?? 'UNKNOWN'} `
        + `new=${item.new_information ?? 'UNKNOWN'} impact=${item.economic_impact ?? 'UNKNOWN'} `
        + `gap=${item.expectation_gap ?? 'UNKNOWN'} pricing=${item.market_pricing ?? 'UNKNOWN'}`,
      )
    }
  }
...
function registerResearchHistoryTool(ctx: Context) {
  return ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{
        type: 'text',
        text: JSON.stringify(value as ResearchHistoryResult, null, 2).slice(0, 30000),
      }],
    },
    async execute(args, exec) {
      return readResearchHistory(args, { signal: exec.signal })
    },
  }))
}
