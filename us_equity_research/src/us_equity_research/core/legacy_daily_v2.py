...
VERSION = "us-daily-review-2"
DISCLAIMER = "本报告仅用于研究，不构成投资建议，所有事实与交易判断须由用户独立复核。"


def analyze(bundle, calendar, sec_snapshot, decision_at):
    ...
    for security in securities:
        ...
        if len(window) == 21 and all(d in series for d in window):
...
def write_review(bundle, calendar, sec_snapshot, decision_at, output_dir):
    packet = analyze(bundle, calendar, sec_snapshot, decision_at)
