...
def analyze(bundle, calendar, sec_snapshot, decision_at, fundamentals=None, filing_bodies=None):
    ...
    from .observation_plan import build_plan

    return {
        "filing_bodies": analyze_bodies(filing_bodies, fundamentals, decision_at),
        "observation_plan": build_plan(
            metrics,
            leaders,
            sector_positive if sector_complete else None,
            len(SECTORS),
            checks,
            latest,
        ),
...
def render(packet):
    ...
    lines = [
        ...
        "| 财报正文与经营验证 | 数字核验见正文；经营语义未完成 | 已接正文数字提取与快照交叉核验，仍需人工核验管理层叙述和命题证伪 |",
        "| 事前收入/EPS一致预期 | 缺失 | 接入获许可、带预测期间和采集版本的预期源；从现在开始留存，不能回填历史预期 |",
        "| TTM、时点股数、债务与估值 | 财务计算与债务核验见正文；估值未知 | 已有期间桥接和时点组件；待补完整债务及同口径价格/股数再计算估值 |",
    ...
    from .observation_plan import render_plan

    plan_lines = render_plan(p["observation_plan"])
    insert_at = lines.index("## 能力检查与缺口处理") - 1
    lines[insert_at:insert_at] = plan_lines
    ordered = sorted(
        p["metrics"].values(),
        key=lambda m: (
            m["symbol"] in SECTORS,
...
def verify_review(directory):
    ...
    if manifest.get("version") == "us-daily-review-5":
        from .legacy_daily_v5 import verify_review as verify_v5

        return verify_v5(directory)
    if manifest.get("version") == "us-daily-review-4":
        from .legacy_daily_v4 import verify_review as verify_v4

        return verify_v4(directory)
