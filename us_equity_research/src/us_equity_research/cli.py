...
def build_parser() -> argparse.ArgumentParser:
    ...
    read_parser.add_argument(
        "--complete", action="store_true", help="Verify and concatenate every artifact page"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            ...
            if args.complete and request.get("offset", 0) != 0:
                raise ContractError("--complete requires offset 0")
            result = read_artifact(request, workspace)
            if args.complete:
                parts = [result["content"]]
                page = result
                while page["next_offset"] is not None:
                    page = read_artifact({**request, "offset": page["next_offset"]}, workspace)
                    parts.append(page["content"])
                result.update(content="".join(parts), truncated=False, next_offset=None)
